...
export async function downloadGDriveFile(
  ...
  const session = axios.create({
    ...
    validateStatus: (status: number) => (status >= 200 && status < 300) || status === 403 // Allow 403 to check for virus scan page
  } as any);
  ...
  if (res1.status === 200 && !ct1.includes('text/html')) {
    return {
      buffer: Buffer.from(res1.data as any),
      contentType: ct1,
      fileName,
    };
  }
  ...
  if (ct1.includes('text/html')) {
    const html = Buffer.from(res1.data as any).toString('utf-8');
