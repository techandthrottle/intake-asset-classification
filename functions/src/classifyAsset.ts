...
function validateResponse(parsed: any, hasVisualContent: boolean): { isValid: boolean; issues: string[] } {
  ...
  if (!parsed.description || typeof parsed.description !== 'string') {
    ...
    const sentenceCount = description.split(/[.!?]+/).filter((s: string) => s.trim().length > 0).length;
...
export const classifyAsset = functions.https.onRequest(async (req, res) => {
  res.set(corsHeaders);
  ...
  try {
    const file: ClassifyRequest = req.body;
