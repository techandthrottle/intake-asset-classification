...
import * as fs from 'fs-extra'; // For temporary file operations
import ffmpeg from 'fluent-ffmpeg';
import ffmpegPath from 'ffmpeg-static';
import { Readable } from 'stream'; // Import Readable for stream piping
// Removed: import { google } from 'googleapis'; // Not needed if using JWT directly
import { JWT } from 'google-auth-library'; // Import JWT for service account
...
interface GoogleDriveFile { // Moved here from processGoogleDrive.ts
  id: string;
  name: string;
  mimeType: string;
...
try {
  ...
  jwtClient = new JWT({ // Using object-based constructor for clarity and robustness
 ...
 */
function buildVideoPrompt(fileName: string): string {
 ...
 */
async function classifyContentWithGemini(
  content: { inlineData?: { mimeType: string; data: string }[] } | { fileData: { fileUri: string } },
  mimeType: string,
  fileName: string,
  geminiApiKey: string, // Changed from apiKey to geminiApiKey
  processingMethod: 'image_direct' | 'video_frames' | 'gcs_registration'
): Promise<Omit<ClassificationResult, 'id' | 'name' | 'mimeType'>> {
  try {
    const genAI = new GoogleGenerativeAI(geminiApiKey); // Use geminiApiKey
    ...
    return {
      ...
      diagnostics: {
        ...
        processingMethod,
        framesExtracted: processingMethod === 'video_frames' ? (content as { inlineData: any[] }).inlineData.length : undefined,
        gcsUri: processingMethod === 'gcs_registration' ? (content as { fileData: { fileUri: string } }).fileData.fileUri : undefined,
      },
    };
 ...
 */
async function downloadGoogleDriveFileContent(fileId: string, accessToken: string, maxSizeBytes: number): Promise<{ data: Buffer, contentType: string }> {
 ...
 */
async function streamDriveFileToGCS(fileId: string, fileName: string, mimeType: string, accessToken: string): Promise<string> {
    const downloadUrl = `https://www.googleapis.com/drive/v3/files/${fileId}?alt=media&confirm=t`;
    ...
    await new Promise<void>((resolve, reject) => {
        ...
        Readable.fromWeb(response.body! as any).pipe(writeStream)
            .on('error', reject)
            .on('finish', () => {
 ...
 */
async function extractFramesFromVideo(videoFilePath: string, videoFileName: string): Promise<Buffer[]> {
    ...
    try {
        const videoProbe = await new Promise<any>((resolve, reject) => {
        ...
        const frameBuffers: Buffer[] = await Promise.all(
            extractedFrameFiles.sort().map(async (frameFileName: string) => {
        ...
        await fs.remove(framesDir).catch((e: unknown) => console.error(`[CLASSIFY_FINAL] Error removing temp frames dir ${framesDir}:`, e));
    }
}


export const classifyAssetFinal = functions.https.onRequest(async (req, res) => {
    res.set(corsHeaders);
    ...
    try {
      ...
      const file: GoogleDriveFile = { id, name, mimeType, size };
 ...
 */
async function processGoogleDriveFile(file: GoogleDriveFile, accessToken: string, geminiApiKey: string): Promise<ClassificationResult> { // Added geminiApiKey parameter
            ...
            try {
                ...
                if (gcsUri) {
                    ...
                    await storage.bucket(GCS_UPLOAD_BUCKET).file(path.basename(gcsUri)).delete()
                        ...
                        .catch((e: unknown) => console.error(`[CLASSIFY_FINAL] Error cleaning up GCS file ${gcsUri}:`, e));
                }
            }
        } 
        // --- Small File (Direct Download/Frame Extraction) Path ---
            ...
            if (file.mimeType.startsWith('image/')) {
                ...
                try {
                    ...
                    await fs.remove(tempVideoFilePath).catch((e: unknown) => console.error(`[CLASSIFY_FINAL] Error removing temp video file ${tempVideoFilePath}:`, e));
                }
            }
        }
    } catch (error) {
